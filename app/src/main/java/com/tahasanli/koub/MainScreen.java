...
public class MainScreen extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding.MainScreenRecycler.setLayoutManager(new LinearLayoutManager(MainScreen.this));
        adapter = new MainScreenAdapter();

        binding.MainScreenRecycler.setAdapter(adapter);

        FirebaseFirestore.getInstance().collection("db").orderBy("Date", Query.Direction.DESCENDING).addSnapshotListener(new EventListener<QuerySnapshot>() {
            @Override
            public void onEvent(@Nullable QuerySnapshot value, @Nullable FirebaseFirestoreException error) {
                if(error != null){
                    Toast.makeText(MainScreen.this, getString(R.string.MainScreenLoadError), Toast.LENGTH_LONG).show();
                    return;
                }
                if(value != null){
